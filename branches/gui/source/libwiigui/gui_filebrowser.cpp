...
GuiFileBrowser::GuiFileBrowser(int w, int h)
{
	...
	{
		fileListText[i] = new GuiText(NULL, 19, (GXColor){35, 166, 234, 0xff});
		fileListText[i]->SetAlignment(ALIGN_LEFT, ALIGN_MIDDLE);
		fileListText[i]->SetPosition(8,2);
		//fileListText[i]->SetMaxWidth(420);
		fileListText[i]->SetMaxWidth(370);
		

		fileListBg[i] = new GuiImage(bgFileSelectionEntry);
		fileListFolder[i] = new GuiImage(fileFolder);
		fileListFolder[i]->SetPosition(0,10);
		
		//fileList[i] = new GuiButton(512,46);
		fileList[i] = new GuiButton(420,46);
		fileList[i]->SetParent(this);
		fileList[i]->SetLabel(fileListText[i]);
		fileList[i]->SetImageOver(fileListBg[i]);
		fileList[i]->SetPosition(2,24*i+3);
		fileList[i]->SetTrigger(trigA);
		//fileList[i]->SetTrigger(trigPlus);
...
int GuiFileBrowser::GetClickedOption()
{
	int found = -1;
	for(int i=0; i<FILE_PAGESIZE; i++)
	{
...
void GuiFileBrowser::Update(GuiTrigger * t)
{
		...
		)
	{
		...
		browser.pageIndex = (positionWiimote * browser.numEntries)/130 - selectedItem;
	...
	for(int i=0; i<FILE_PAGESIZE; i++)
	{
		if(listChanged || numEntries != browser.numEntries)
		{
			if(browser.pageIndex+i < browser.numEntries)
			{
				...
				char tempname[256];
				if (checkExt(browserList[browser.pageIndex+i].displayname,"wad"))
				{
				long tempsize = ((browserList[browser.pageIndex+i].length-7340032)/1024)/128;
				//sprintf(tempname,"%s Blocks:%li",browserList[browser.pageIndex+i].displayname,tempsize);
				sprintf(tempname,"%s",browserList[browser.pageIndex+i].displayname);
				...
				fileListText[i]->SetText(tempname);
		...
		if(fileList[i]->GetState() == STATE_SELECTED)
		{
			selectedItem = i;
			browser.selIndex = browser.pageIndex + i;
			//WindowPrompt("Selected File",(const char*)fileList[browser.selIndex],"Back",0);
		}

		if(fileList[i]->GetState() == STATE_SELECTED && (t->wpad.btns_d & WPAD_BUTTON_PLUS))
		{
			selectedItem = i;
			browser.selIndex = browser.pageIndex + i;
			//WindowPrompt("Selected File",(const char*)fileList[browser.selIndex],"Back",0);
	...
	{
		position = (height-118)*(browser.pageIndex + FILE_PAGESIZE/2.0) / (browser.numEntries*1.0);
